...
当前 RouteDoctor 的 Ping 监控页面仅支持同时监控单个目标。本功能扩展 Ping 页面，使其支持以标签页（Tab）形式同时监控多个目标（如 8.8.8.8、google.com、baidu.com 等），每个目标拥有独立的配置、图表、统计和结果表格。后端 ICMP 服务已支持多 session 并发管理，本功能的核心工作集中在前端 UI 层面。
